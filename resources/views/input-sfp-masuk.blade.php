...
@section('content')
    ...
    <div class="row h-100">
        <div class="col-lg-6 m-auto">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Input Sfp Masuk</h5>
                    <form action="sfp" method="post" class="row g-3 main_form">
                        @csrf
                        <div class="mt-3">
                            <label for="jenis" class="form-label">Jenis</label>
                            <select name="jenis" id="jenis" class="form-control" value="{{ old('jenis') }}">
                                <option selected disabled>Pilih Jenis</option>
                                <option value="SFP" {{ old('jenis') === 'SFP' ? 'selected' : '' }}>SFP</option>
                                <option value="SFP+" {{ old('jenis') === 'SFP+' ? 'selected' : '' }}>SFP+</option>
                                <option value="XFP" {{ old('jenis') === 'XFP' ? 'selected' : '' }}>XFP</option>
                                <option value="QSFP" {{ old('jenis') === 'QSFP' ? 'selected' : '' }}>QSFP</option>
                            </select>
                            <span class="text-danger error-text jenis_error"></span>
                        </div>
                        <div class="mt-3">
                            <label for="vendor" class="form-label">Vendor</label>
                            <input type="text" class="form-control" id="vendor" name="vendor" value="{{ old('vendor') }}" style="text-transform:uppercase">
                            <span class="text-danger error-text vendor_error"></span>
                        </div>
                        <div class="mt-3">
                            <label for="bandwidth" class="form-label">Bandwidth (G)</label>
                            <input type="text" class="form-control" id="bandwidth" name="bandwidth" readonly value={{ old('bandwidth') ? "old('bandwidth')" : '' }}>
                            <span class="text-danger error-text bandwidth_error"></span>
                        </div>
                        <div class="mt-3">
                            <label for="lambda" class="form-label">Lambda (nm)</label>
                            <input type="text" class="form-control" id="lambda" name="lambda" readonly value={{ old('lambda') ? "old('lambda')" : '' }}>
                            <span class="text-danger error-text lambda_error"></span>
                        </div>
                        <div class="mt-3">
                            <label for="jarak" class="form-label">Jarak (KM)</label>
                            <select name="jarak" id="jarak" class="form-control" value="{{ old('jarak') }}">
                                <option selected disabled>Pilih Jarak</option>
                                <option value="10" {{ old('jarak') === '10' ? 'selected' : '' }}>10</option>
                                <option value="40" {{ old('jarak') === '40' ? 'selected' : '' }}>40</option>
                                <option value="80" {{ old('jarak') === '80' ? 'selected' : '' }}>80</option>
                            </select>
                        ...
                        <div class="mt-3">
                            ...
                            <span class="text-danger error-text serial_number_error" id="serial_number_error"></span>
